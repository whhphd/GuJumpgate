...
- CPA / SUB2API 配置
- Codex2API 配置
- IP 代理持久配置：`ipProxyEnabled`、`ipProxyScope`（`incognito_persistent` / `regular`，默认 `incognito_persistent`）、服务商、模式、API 地址、服务商配置快照、账号列表、固定 Host / Port / Protocol / Username / Password、地区参数、session 与自动切换阈值
...
10. `chrome.proxy.settings` 的 set / clear 都按当前 `ipProxyScope` 解析后的目标作用域执行；默认 `incognito_persistent` 仅接管无痕窗口，可在侧边栏切换为 `regular`（同时接管普通窗口）；apply 成功后会主动 clear 另一个作用域的旧 PAC，避免切换 scope 后旧设置继续残留。
11. 扩展自身 service worker 后台 fetch 不属于任何标签窗口，不会被 `chrome.proxy` 的作用域控制；这条链路只影响标签页流量。
12. `IP_PROXY_FORCE_DIRECT_HOST_PATTERNS` 默认包含支付转换相关域名（`pm-redirects.stripe.com / hwork.pro / luckyous.com`）；OpenAI 三个授权域 `auth.openai.com / auth0.openai.com / accounts.openai.com` 不在 force-direct 列表中，会真实经过代理出口。
...
- 新增或调整 `ipProxyScope` 默认值时，必须同时检查 `PERSISTED_SETTING_DEFAULTS`、`normalizePersistentSettingValue`、sidepanel 回显与 `applyIpProxySettingsFromState` 的目标作用域解析是否一致，并确认无痕窗口对扩展是否仍开启权限的提示文案准确。
## 8. 自动运行完整链路
