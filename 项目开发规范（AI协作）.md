...
- 自动账号、卡密、号码、代理等队列流程必须区分“检测/网络/页面失败”和“目标已确认不可用”。只有目标被业务结果明确判定为坏（如卡密无效、已使用、平台明确拒绝）时，才允许删除或淘汰队列项；网络错误、Cloudflare、页面解析失败、窗口/标签页失效、超时、用户手动停止等必须保留当前队列项并暂停或等待重试。
- 对异步回填的卡密/取码页面，点击“换出”“取码”等按钮后必须等待页面完成回填，并优先读取当前表单字段里的新值；不要从整页文本兜底提取邮箱，避免读到上一轮残留邮箱后污染 OAuth 登录账号。
- 卡密/取码站在“换出邮箱/秘钥”和“邮箱取码”两个动作里出现 `Failed to fetch`、网络瞬断或页面请求失败时，都必须优先重新点击、刷新页面并继续重试当前卡密；不要直接暂停队列，更不能把旧页面残留邮箱或旧验证码当成当前卡密结果。
- Plus 卡密流程在已拿到 OpenAI localhost callback 后，如果 SUB2API 导入阶段只是 `/api/v1/auth/login`、`/api/v1/admin/openai/exchange-code` 或创建账号请求的网络/超时类失败，必须先短超时多次重试同一个 callback；最终仍失败时，应保留当前卡密/邮箱为“导入待重试”并继续无人值守处理后续卡密，不能直接删除当前卡密，也不能把整个队列停死。
- Plus 卡密取码站和 SUB2API 管理接口属于控制面请求，必须通过 PAC `DIRECT` 绕过插件任务代理；不要让 `plus.keria.cc.cd`、当前 `sub2apiUrl` 或 `/api/v1/admin/openai/exchange-code` 依赖 OpenAI 任务代理 IP。
- 新增从中段节点启动的专用流程时，必须逐项核对被跳过前置节点里的清理、隔离、锁定和运行态初始化能力；例如 Plus 卡密流程从 OAuth 链路开始，也必须复用 OpenAI / ChatGPT cookies 清理，避免多个账号会话残留叠加。
