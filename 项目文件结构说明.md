...
- `background.js`：扩展后台 Service Worker 入口壳，负责模块装配、初始化、全局常量、少量保留的领域函数与运行入口；当前新增 `oauthFlowTimeoutEnabled` 持久化配置，并统一承接 OAuth 授权后链总预算开关与剩余预算计算；当前还承接 IP 代理作用域 `ipProxyScope`（默认 `incognito_persistent`，仅无痕窗口接管；可选 `regular`，所有窗口接管）的归一化与默认值。
...
- `background/ip-proxy-core.js`：IP 代理核心模块，负责代理条目解析、账号/接口代理池运行态、PAC 应用与清除、代理鉴权回填、出口探测、失败时的目标站点 fail-close 防漏规则，以及自动运行成功阈值后的代理切换支撑；当前 `chrome.proxy.settings` 的 set / clear / drain 都按当前 `ipProxyScope` 解析的目标作用域执行，apply 成功后会主动清掉另一个作用域的旧 PAC，避免切换 scope 后旧设置继续接管另一类窗口。
