...
本次版本聚焦三件事：把 SMS OAuth 流程改成更贴近真实页面状态的执行顺序，补齐更多可 API 化的邮箱/接码供应源，并把 PayPal Hosted Checkout、手机号验证和批量自动运行中的高频中断点做成可恢复、可重试、可观察的链路。
...
- 邮箱源扩展到更多可 API 轮询场景：
  - 新增 `freemail`，支持后台登录、拉取域名、生成邮箱、直接 API 轮询验证码。
  - 新增 `Outlook Email Plus`，支持邮箱池认领、验证码轮询、认领释放，以及 PayPal 别名复用。
